...
if nargin < 1
    error(['Input network is not defined. Please use ''Netbuild'''...
           'to construct the input network first.'])
end
...
trainRatio = ceil(size(INdata,1) * ratioValue * 0.01);
...
% Feedforward process
    for epoch = 1:fineTrainEpoch
        ...
        for num = 1:length(batchIndex)
            ...
            if strcmp(outputAct,'sigmoid')
                layerMemory{hiddenLayerNumber-1} = BinarySigmoid(layerStore);
    ...
    if strcmp(preTrain,'on');
        fprintf('Fine-tuning complete.\nDNN process has been finished.\n\n')
    else fprintf('DNN process has been finished.\n\n');
    end
